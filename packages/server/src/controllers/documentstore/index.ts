...
const previewFileChunks = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (body.storeId) {
            ...
            if (!store) {
                throw new InternalAccelanceError(StatusCodes.NOT_FOUND, 'Document store not found')
            }
        }
        body.preview = true
...
const insertIntoVectorStore = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        getRunningExpressApp().metricsProvider?.incrementCounter(ACCELANCE_METRIC_COUNTERS.VECTORSTORE_UPSERT, {
