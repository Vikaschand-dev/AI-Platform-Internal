...
function validateWithZod<T>(schema: z.ZodSchema<T>, data: unknown): T {
    ...
    if (!result.success) {
        throw new InternalAccelanceError(StatusCodes.BAD_REQUEST, result.error.errors[0].message)
    }
    return result.data
}
...
const updateMcpServerConfig = async (
    ...
    try {
        ...
        const existing = parseMcpConfig(chatflow)
        if (!existing) {
            throw new InternalAccelanceError(StatusCodes.NOT_FOUND, `MCP server config not found for ID: ${chatflowId}`)
        }
...
const getChatflowByIdAndVerifyToken = async (chatflowId: string, token: string): Promise<ChatFlow> => {
    try {
        ...
        if (!chatflow) {
            throw new InternalAccelanceError(StatusCodes.NOT_FOUND, 'MCP server not found')
        }
        ...
        if (storedBuffer.length !== providedBuffer.length || !crypto.timingSafeEqual(storedBuffer, providedBuffer)) {
            throw new InternalAccelanceError(StatusCodes.UNAUTHORIZED, 'Invalid token')
        }
