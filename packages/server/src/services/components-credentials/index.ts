    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(appServer.nodesPool.componentCredentials, credentialName)) {
            ...
            if (credInstance.icon === undefined) {
                throw new InternalAccelanceError(StatusCodes.NOT_FOUND, `Credential ${credentialName} icon not found`)
            }
            ...
            if (credInstance.icon.endsWith('.svg') || credInstance.icon.endsWith('.png') || credInstance.icon.endsWith('.jpg')) {
                ...
                throw new InternalAccelanceError(StatusCodes.INTERNAL_SERVER_ERROR, `Credential ${credentialName} icon is missing icon`)
            }
        } else {
            throw new InternalAccelanceError(StatusCodes.NOT_FOUND, `Credential ${credentialName} not found`)
        }
    } catch (error) {
        throw new InternalAccelanceError(
            StatusCodes.INTERNAL_SERVER_ERROR,
